...
for i in range(xdiv):
    for j in range(ydiv):
        ...
        ne = {str(xmin), str(ymax)}
        sw = {str(xmax), str(ymin)}
        res = str(i)+str(j)
        print('{"file:"')
        print('"' + res + '",')
        print("ne:", ne)
        ...
        # gdal.Translate("raster_translate"+str(i)+str(j)+".tif", dem, projWin = (xmin, ymax, xmax, ymin), xRes = res, yRes = -res)
